...
run.logParam("maxDepth",maxDepth.toString)
run.logParam("maxBins",maxBins.toString)
println("Parameters:")
...
val file = Paths.get("tree_model.txt")
val content = s"Learned regression tree model - ${new java.util.Date()}"+s"\n${treeModel.toDebugString}"
Files.write(file, content.getBytes)
run.logArtifact(file, "info")

// MLflow - save models
dbutils.fs.rm(modelDir,true)
saveModelAsSparkMLContext(run, modelDir, model)
saveModelAsMLeapContext(run, modelDir, model, predictions)
