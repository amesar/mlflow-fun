...
class BuildTables(object):
    ...
    def get_values(self, obj):
        ...
        values = [ x or 0 for x in values ] # TODO: Hack to ensure end_time stays long since a single None makes the column a string ;)
        return values
    ...
    def build_run_table(self, exps):
        ...
        for j,exp in enumerate(exps):
            try:
                it = SearchRunsIterator(mlflow_client, [exp.experiment_id], MAX_RESULTS)
                print("{}/{} Experiment {} - {}".format((1+j),len(exps),exp.experiment_id,exp.name))
                for j,run in enumerate(it):
                    if run_row is None:
                ...
                print("{}/{} Experiment {} has {} runs - {}".format((1+j),len(exps),exp.experiment_id,j,exp.name))
